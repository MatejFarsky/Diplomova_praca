...
for i in range(len(exe_unique)):
    ...
    for j in range(len(restrict)):
        curr_time = pd.to_datetime(exe_unique[100], format='%Y.%m.%d %H:%M')
        if curr_time.day == 3:
            restrict[j] = 1
        if (curr_time.hour * 60 + curr_time.minute) < (
                pd.to_datetime(df1.time[j]).hour * 60 + pd.to_datetime(df1.time[j]).minute - 30):
            restrict[j] = 1
