...
def load_data(date):
    ...
    spot_prices = pd.read_excel('Spot_prices.xlsx',sheet_name=date)
...
def calculate_day(date):
    df_filter, exe_unique, df1, tradab, mean_prices, spot_prices = load_data(date)
    df2 = pd.DataFrame({
        'ExeTime': 'Spot',
        'Profit': [0]
    })
