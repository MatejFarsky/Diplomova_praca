...
restrict = pd.Series(np.ones(len(df1.price_bid)))
...
for i in range(len(exe_unique)):
    ...
    df1.loc[df1['time'] == current['DeliveryStartTime'].iloc[0], 'price_bid'] = 0.995 * float(current.Price.iloc[0].replace(',', '.'))
    df1.loc[df1['time'] == current['DeliveryStartTime'].iloc[0], 'price_ask'] = 1.005 * float(current.Price.iloc[0].replace(',', '.'))
