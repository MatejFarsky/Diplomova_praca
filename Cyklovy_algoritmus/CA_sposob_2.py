...
def iteration(df1,df):
    ...
    for k in range(len(df.buy_prev)):
        for j in range(len(df.buy_prev)-1,k,-1):
            df_try = df1.copy()
            df_try.loc[df_try.time == df.time[k], 'buy_prev'] = df.buy_prev[j]
            df_try.loc[df_try.time == df.time[k],'sell_prev'] = df.sell_prev[j]
            df_try.loc[df_try.time == df.time[j],'buy_prev'] = df.buy_prev[k]
            df_try.loc[df_try.time == df.time[j],'sell_prev'] = df.sell_prev[k]
            ...
            profit += df_try.loc[df_try.time == df.time[j],'price'].values[0]*(df_try.loc[df_try.time == df.time[j],'sell_prev'].values[0] - df_try.loc[df_try.time == df.time[j], 'buy_prev'].values[0])
            ...
            profit_orig += df1.loc[df1.time == df.time[j],'price'].values[0]*(df1.loc[df1.time == df.time[j],'sell_prev'].values[0] - df1.loc[df1.time == df.time[j], 'buy_prev'].values[0])
...
for i in range(len(exe_unique)):
    ...
    if len(df.time)>1:
        ...
        df2.loc[len(df2.index)] = [unique(current.loc[:,'ExecutionTime']), opt_val]
